...
export default function CandidateTopics({
  ...
  return (
    ...
    >
      {/* Add topic input — always visible at the top */}
      <div className={`px-3 pt-3 pb-2 border-b ${isDark ? "border-zinc-800" : "border-gray-100"}`}>
          ...
          <input
            ...
            className={`flex-1 px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-400 text-sm ${
              isDark
                ...
                : "border-gray-200 placeholder-gray-400 text-gray-800"
            }`}
            onKeyDown={(e) => {
              if (e.key === "Enter") handleAddCandidate();
              else if (e.key === "Escape") setNewCandidate("");
            }}
          />
          <button
        ...
        {selectedCount > 0 && (
          <p className="text-xs text-indigo-600 font-medium mt-1.5 pl-0.5">
            {selectedCount} selected for convergence
          </p>
        )}
      </div>
